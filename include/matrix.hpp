...
template<typename ValueT>
Matrix<ValueT> Matrix<ValueT>::edge_reflection(uint radius) const {
    // Generation bigger image using reflections outside the edges
    if (n_cols < radius || n_rows < radius)
        throw std::string("Invalid matrix size or radius");

    Matrix<ValueT> big(n_rows + 2 * radius, n_cols + 2 * radius);
    for (uint i = 0; i < big.n_rows; i++)
        for (uint j = 0; j < big.n_cols; j++) {
            if (i < radius) {
                if (j < radius)
                    big(i, j) = (*this)(radius - i, radius - j);
                if (radius <= j && j < n_cols + radius)
                    big(i, j) = (*this)(radius - i, j - radius);
                if (n_cols + radius <= j)
                    big(i, j) = (*this)(radius - i, n_cols - (j - n_cols - radius + 1));
            }
            if (radius <= i && i < big.n_rows - radius) {
                if (j < radius)
                    big(i, j) = (*this)(i - radius, radius - j);
                if (radius <= j && j < n_cols + radius)
                    big(i, j) = (*this)(i - radius, j - radius);
                if (n_cols + radius <= j)
                    big(i, j) = (*this)(i - radius, n_cols - (j - n_cols - radius + 1));
            }
            if (big.n_rows - radius <= i) {
                if (j < radius)
                    big(i, j) = (*this)(n_rows - (i - n_rows - radius + 1), radius - j);
                if (radius <= j && j < n_cols + radius)
                    big(i, j) = (*this)(n_rows - (i - n_rows - radius + 1), j - radius);
                if (n_cols + radius <= j)
                    big(i, j) = (*this)(n_rows - (i - n_rows - radius + 1), n_cols - (j - n_cols - radius + 1));
            }
        }
    return big;
}
...
template<typename ValueT, typename BinaryMatrixOperator>
Matrix<typename std::result_of<BinaryMatrixOperator(Matrix<ValueT>, Matrix<ValueT>)>::type>
binary_map(const BinaryMatrixOperator &op,
    const Matrix<ValueT> m1, const Matrix<ValueT> m2) {

    typedef typename std::result_of<BinaryMatrixOperator(Matrix<ValueT>, Matrix<ValueT>)>::type ReturnT;
    if (m1.n_rows != m2.n_rows || m1.n_cols != m2.n_cols)
        throw std::string("Invalid binary_map operands");
    if (m1.n_rows * m1.n_cols == 0)
        return Matrix<ReturnT>(0, 0);

    Matrix<ReturnT> tmp(m1.n_rows, m1.n_cols);

    const auto radius1 = op.radius1;
    const auto radius2 = op.radius2;
    const auto size1 = 2 * radius1 + 1;
    const auto size2 = 2 * radius2 + 1;
    const auto radius = std::max(radius1, radius2);

    const auto start_i = radius;
    const auto end_i = m1.n_rows - radius;
    const auto start_j = radius;
    const auto end_j = m1.n_cols - radius;

    for (uint i = start_i; i < end_i; i++)
        for (uint j = start_j; j < end_j; ++j) {
            auto neighbourhood1 = m1.submatrix(i - radius1, j - radius1, size1, size1);
            auto neighbourhood2 = m2.submatrix(i - radius2, j - radius2, size2, size2);
            tmp(i, j) = op(neighbourhood1, neighbourhood2);
        }
    return tmp;
}
...
template<typename ValueT, typename BinaryMatrixOperator>
Matrix<typename std::result_of<BinaryMatrixOperator(Matrix<ValueT>, Matrix<ValueT>)>::type>
binary_map(BinaryMatrixOperator &op,
    const Matrix<ValueT> m1, const Matrix<ValueT> m2) {
